...
static u32 set_mario_action_airborne(struct MarioState *m, u32 action, u32 actionArg) {
    ...
    switch (action) {
        case ACT_DOUBLE_JUMP:
            set_mario_y_vel_based_on_fspeed(m, 40.0f, 0.20f);
            m->forwardVel *= 0.6f;
            break;
        ...
        case ACT_TRIPLE_JUMP:
            set_mario_y_vel_based_on_fspeed(m, 53.0f, 0.0f);
            m->forwardVel *= 0.8f;
            break;
        ...
        case ACT_HOLD_WATER_JUMP:
            if (actionArg == 0) {
                set_mario_y_vel_based_on_fspeed(m, 19.0f, 0.0f);
            }
            break;
        ...
        case ACT_RIDING_SHELL_JUMP:
            set_mario_y_vel_based_on_fspeed(m, 22.0f, 0.22f);
            break;

        case ACT_JUMP:
        case ACT_HOLD_JUMP:
            m->marioObj->header.gfx.animInfo.animID = -1;
            set_mario_y_vel_based_on_fspeed(m, 31.0f, 0.20f);
            m->forwardVel *= 0.8f;
            break;

        case ACT_WALL_KICK_AIR:
        case ACT_TOP_OF_POLE_JUMP:
            set_mario_y_vel_based_on_fspeed(m, 20.0f, 0.0f);
        ...
        case ACT_SIDE_FLIP:
            set_mario_y_vel_based_on_fspeed(m, 52.1f, 21.0f);
            m->forwardVel = 21.0f;
        ...
        case ACT_STEEP_JUMP:
            m->marioObj->header.gfx.animInfo.animID = -1;
            set_mario_y_vel_based_on_fspeed(m, 22.0f, 0.15f);
        ...
        case ACT_DIVE:
            if ((forwardVel = m->forwardVel + 25.0f) > 52.0f) {
                forwardVel = 52.0f;
            }
        ...
        case ACT_LONG_JUMP:
            m->marioObj->header.gfx.animInfo.animID = -1;
            set_mario_y_vel_based_on_fspeed(m, 41.0f, 0.0f);
            m->marioObj->oMarioLongJumpIsSlow = m->forwardVel > 23.0f ? FALSE : TRUE;

            //! (BLJ's) This properly handles long jumps from getting forward speed with
            //  too much velocity, but misses backwards longs allowing high negative speeds.
            if ((m->forwardVel *= 2.9f) > 22.0f) {
                m->forwardVel = 22.0f;
            }
            break;
